"""Minor changes to support Python3 & IDA 7.7 - mzpqnxow"""
import collections
import idautils
import logging
import idaapi
...
class ConfigStingray( idaapi.action_handler_t ):
    ...
    def save():
        ...
        open(ConfigStingray.CONFIG_FILE_PATH,"w").write(config_data)


    @staticmethod
    def stingray_config():

        input = ida_kernwin.ask_long(ConfigStingray.SEARCH_RECURSION_MAXLVL, 
...
class String( object ):
    ...
    def __init__( self, xref, addr ):
        ...
        if type is None:
            raise StringParsingException('Not a string')
        if type < 0 or type >= len(String.ASCSTR):
            raise StringParsingException()

        CALC_MAX_LEN = -1
        string = str( idc.get_strlit_contents(addr, CALC_MAX_LEN, type) )
...
class PluginChooser( idaapi.Choose ):
