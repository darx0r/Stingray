...
class ConfigStingray(idaapi.action_handler_t):
    ...
    CONFIG_FILE_PATH = os.path.join(idc.idadir(), "cfg/Stingray.cfg")

    CHOOSER_TITLE = "Stingray - Function Strings"
    CHOOSER_COLUMN_NAMES = ["Xref", "Address", "Type", "String"]
    CHOOSER_COLUMN_SIZES = [18, 8, 5, 80]
    CHOOSER_COLUMNS = [list(c) for c in zip(CHOOSER_COLUMN_NAMES, CHOOSER_COLUMN_SIZES)]
    ...
    PLUGIN_ICON_PNG = (
        ...
        b"\x00\x00\x00\x10\x00\x00\x00\x10\x08\x04\x00\x00\x00\xB5\xFA\x37"
        b"\xEA\x00\x00\x00\x02\x62\x4B\x47\x44\x00\xFF\x87\x8F\xCC\xBF\x00"
        b"\x00\x00\x09\x70\x48\x59\x73\x00\x00\x00\x48\x00\x00\x00\x48\x00"
        b"\x46\xC9\x6B\x3E\x00\x00\x00\xC4\x49\x44\x41\x54\x28\xCF\xBD\x91"
        b"\x3D\x0B\x81\x61\x14\x86\xAF\xE7\xF5\xCA\x77\xCA\x20\x0B\x61\xA0"
        b"\x58\x8C\x06\x65\xF4\x11\x99\x8C\xDE\x92\x18\x64\xF6\x13\xFC\x03"
        b"\x25\x59\xEC\x7E\x80\x32\xDB\x2C\x36\x61\x92\x85\x59\x49\xD4\xE3"
        b"\xF1\x35\x48\x8C\xCE\x5D\x67\x39\xD7\xE9\xDC\xE7\x1C\xF8\x4B\xB8"
        b"\xF1\xFD\x06\x8A\xD4\xBE\xD6\x4C\xB7\xD4\x61\x46\xE2\xA3\x64\xC6"
        b"\x4B\x8C\x00\x08\x86\x48\xE6\xD4\x49\x13\xA5\x80\x41\x86\xBC\x52"
        b"\x8E\x2C\x71\x2C\x37\xB6\xAD\x00\xC9\x85\x03\x5B\x8E\x0C\x08\x12"
        b"\x56\xAE\xEC\xAA\xF5\x1E\x42\x33\xF4\x93\x90\xBC\xD4\x47\x7B\x9F"
        b"\x55\xA2\x2B\xF6\x56\xA9\xBD\x80\x0D\xA9\x77\xC0\x85\x8D\x8A\x98"
        b"\x30\x62\xC5\xF9\x8E\x2C\x69\x12\x79\x8E\x70\x3C\x20\x8F\x92\x4E"
        b"\x88\xAA\x32\x3C\x65\xCD\x8E\x05\x63\x7A\xB4\x28\x7F\xEE\xED\xC6"
        b"\xAF\x96\x2E\xA8\xDB\x34\x48\xE2\xFC\xC3\xA3\xAE\x2A\xB7\x2E\x12"
        b"\xA0\x06\x96\xA7\x00\x00\x00\x25\x74\x45\x58\x74\x64\x61\x74\x65"
        b"\x3A\x63\x72\x65\x61\x74\x65\x00\x32\x30\x31\x35\x2D\x31\x30\x2D"
        b"\x30\x31\x54\x31\x31\x3A\x35\x34\x3A\x33\x38\x2D\x30\x35\x3A\x30"
        b"\x30\x9E\x3D\x6C\xB8\x00\x00\x00\x25\x74\x45\x58\x74\x64\x61\x74"
        b"\x65\x3A\x6D\x6F\x64\x69\x66\x79\x00\x32\x30\x31\x35\x2D\x31\x30"
        b"\x2D\x30\x31\x54\x31\x31\x3A\x35\x34\x3A\x33\x38\x2D\x30\x35\x3A"
        b"\x30\x30\xEF\x60\xD4\x04\x00\x00\x00\x00\x49\x45\x4E\x44\xAE\x42"
        b"\x60\x82"
    )
    ...
    def save():
        ...
        open(ConfigStingray.CONFIG_FILE_PATH, "wb").write(config_data.encode())

    @staticmethod
    def stingray_config():

        input = idc.AskLong(
            ConfigStingray.SEARCH_RECURSION_MAXLVL, "Please enter string search max. depth:" "\n( 0 - non-recursive mode )"
        )
...
def find_function_callees(func_ea, maxlvl):
    ...
    while len(pending) > 0:
        ...
        for line in idautils.Heads(func_ea, func_end):
            ...
            if not ida_bytes.isCode(get_flags(line)):
                continue

            ALL_XREFS = 0
            refs = idautils.CodeRefsFrom(line, ALL_XREFS)
            refs = set([x for x in refs if not (x >= func_ea and x <= func_end)])
            all_refs |= refs
...
class StringFinder(object):
    ...
    def get_current_function_strings(self):
        ...
        if ConfigStingray.SEARCH_RECURSION_MAXLVL > 0:
            funcs = find_function_callees(addr_in_func, ConfigStingray.SEARCH_RECURSION_MAXLVL)
    ...
    def run(self, arg=0):
        ...
        try:
            ...
            if self._chooser is None:
                self._chooser = PluginChooser(ConfigStingray.CHOOSER_TITLE, ConfigStingray.CHOOSER_COLUMNS, rows, self.icon_id)
            else:
